/* Process model C form file: manet_dispatcher_cluster.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char manet_dispatcher_cluster_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A modeler 7 54F4927E 54F4927E 1 hp-PC hp 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1e80 8                                                                                                                                                                                                                                                                                                                                                                                                                 ";
#include <string.h>
...
#define     Send_msg             2801




#define     Go_To_send            (intrpt_code==Send_msg)
...
typedef struct
	...
	} manet_dispatcher_cluster_state;
...
void send_pkt()
	{
		Objid nodeid, procid,my_id,my_nid,my_sid;
		Packet * pkptr;
		pkptr=op_pk_create(64);
		op_pk_fd_set (pkptr, 0, OPC_FIELD_TYPE_INTEGER, 2511, 16);
		my_id         = op_id_self ();
		my_nid    = op_topo_parent (my_id);
		my_sid  = op_topo_parent (my_nid);
		nodeid=op_topo_child (my_sid,OPC_OBJTYPE_NODE_FIX ,0);
		int count=op_topo_child_count (nodeid, OPC_OBJTYPE_PROC);
		cout<<count;

		//procid=op_topo_child (nodeid,OPC_OBJTYPE_PROC ,0);
		//op_pk_deliver(pkptr,nodeid,0);
...
void
manet_dispatcher_cluster (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (manet_dispatcher_cluster ());
		...
		FSM_BLOCK_SWITCH
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 3, state3_enter_exec, manet_rpg_packet_destroy ();, "STREAM_INTERRUPT", "manet_rpg_packet_destroy ()", "dispatch", "dispatch", "tr_38", "manet_dispatcher_cluster [dispatch -> dispatch : STREAM_INTERRUPT / manet_rpg_packet_destroy ()]")
				FSM_CASE_TRANSIT (1, 3, state3_enter_exec, send_pkt ();, "SELF_INTERRUPT", "send_pkt ()", "dispatch", "dispatch", "tr_36", "manet_dispatcher_cluster [dispatch -> dispatch : SELF_INTERRUPT / send_pkt ()]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (4, "wait_2", state4_enter_exec, "manet_dispatcher_cluster [wait_2 enter execs]")
				...
				/** IP destinations.								**/
				op_intrpt_schedule_self (op_sim_time ()+2, Go_To_send);
				}
...
void
_op_manet_dispatcher_cluster_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_manet_dispatcher_cluster_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_manet_dispatcher_cluster_terminate ())
...
/* local variable prs_ptr in _op_manet_dispatcher_cluster_svar function. */
#undef my_objid
#undef my_node_objid
...
_op_manet_dispatcher_cluster_init (int * init_block_ptr)
	...
	obtype = Vos_Define_Object_Prstate ("proc state vars (manet_dispatcher_cluster)",
...
VosT_Address
_op_manet_dispatcher_cluster_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	manet_dispatcher_cluster_state * ptr;
	FIN_MT (_op_manet_dispatcher_cluster_alloc (obtype))

	ptr = (manet_dispatcher_cluster_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "manet_dispatcher_cluster [init enter execs]";
#endif
		}
