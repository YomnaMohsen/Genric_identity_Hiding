/* Process model C++ form file: ip_arp_v4_malc.pr.cpp */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char ip_arp_v4_malc_pr_cpp [] = "MIL_3_Tfile_Hdr_ 145A 30A modeler 7 5944E3FD 5944E3FD 1 hp-PC hp 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1e80 8                                                                                                                                                                                                                                                                                                                                                                                                                 ";
#include <string.h>
...
class ip_arp_v4_malc_state
	...
	public:
		...
		/* FSM code */
		void ip_arp_v4_malc (OP_SIM_CONTEXT_ARG_OPT);
		/* Diagnostic Block */
		void _op_ip_arp_v4_malc_diag (OP_SIM_CONTEXT_ARG_OPT);
...
VosT_Obtype ip_arp_v4_malc_state::obtype = (VosT_Obtype)OPC_NIL;
...
#define FIN_PREAMBLE_DEC	ip_arp_v4_malc_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((ip_arp_v4_malc_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
arp_packet_from_ip_handle (int intrpt_strm)
	...
	ip_comp_row_objid = op_topo_child (ip_comp_objid, OPC_OBJTYPE_GENERIC, 0);
	op_ima_obj_attr_get (ip_comp_row_objid, "Address", orgaddarr);
	//cout<<"ADDR"<< orgaddarr<<"\n";

	
		orgadd=prg_ip_address_string_to_value (orgaddarr);
	...
	if(malci)
		...
		ip_dgram_fd_ptr->src_addr.address.ipv4_addr=spoofed;
		
		
		}
	else if(!malci)
		{
		ip_dgram_fd_ptr->src_addr.address.ipv4_addr=orgadd;
	...
	//}
		...
		//if(my_node_objid==3/*my_node_objid==26||my_node_objid==9||my_node_objid==9||my_node_objid==18||my_node_objid==20*/)
...
arp_ip_packet_from_mac_handle (Packet* pkptr, int subintf_index)
	...
	/* be invalid. In this case the
	packet should simply	*/
	/* be destroyed 										*/
	...
	//cout<<"string"<<mask<<"\n";
			...
			if(check_range(ip_dgram_fd_ptr_temp->src_addr.address.ipv4_addr))
			{
			
			//cout<<"in mac "<<1<<"\n";
			spoofed=ip_dgram_fd_ptr_temp->src_addr.address.ipv4_addr;
...
static void initstat()
	{
	FIN (init_stat);
	global_pkt_drop=op_stat_reg("glo_pkt_drop",OPC_STAT_INDEX_NONE, OPC_STAT_GLOBAL);
	FOUT;
	}
static void pkt_drop()
	{
...
extern "C"
{
	VosT_Obtype _op_ip_arp_v4_malc_init (int * init_block_ptr);
	VosT_Address _op_ip_arp_v4_malc_alloc (VosT_Obtype, int);
	void ip_arp_v4_malc (OP_SIM_CONTEXT_ARG_OPT)
		{
		((ip_arp_v4_malc_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->ip_arp_v4_malc (OP_SIM_CONTEXT_PTR_OPT);
		}

	void _op_ip_arp_v4_malc_svar (void *, const char *, void **);

	void _op_ip_arp_v4_malc_diag (OP_SIM_CONTEXT_ARG_OPT)
		{
		((ip_arp_v4_malc_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->_op_ip_arp_v4_malc_diag (OP_SIM_CONTEXT_PTR_OPT);
		}

	void _op_ip_arp_v4_malc_terminate (OP_SIM_CONTEXT_ARG_OPT)
		{
		/* The destructor is the Termination Block */
		delete (ip_arp_v4_malc_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr);
		}
...
void
ip_arp_v4_malc_state::ip_arp_v4_malc (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (ip_arp_v4_malc_state::ip_arp_v4_malc ());
	try
		{
		/* Temporary Variables */
		...
		FSM_ENTER ("ip_arp_v4_malc")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (INIT) enter executives **/
			FSM_STATE_ENTER_UNFORCED_NOLABEL (0, "INIT", "ip_arp_v4_malc [INIT enter execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_malc [INIT enter execs]", state0_enter_exec)
				{
			...
			FSM_EXIT (1,"ip_arp_v4_malc")


			/** state (INIT) exit executives **/
			FSM_STATE_EXIT_UNFORCED (0, "INIT", "ip_arp_v4_malc [INIT exit execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_malc [INIT exit execs]", state0_exit_exec)
				{
			...
			FSM_TRANSIT_ONLY ((SELF_NOTIF), 3, state3_enter_exec, ;, INIT, "SELF_NOTIF", "", "INIT", "wait", "tr_35", "ip_arp_v4_malc [INIT -> wait : SELF_NOTIF / ]")
				/*---------------------------------------------------------*/



			/** state (WAIT) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "WAIT", state1_enter_exec, "ip_arp_v4_malc [WAIT enter execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_malc [WAIT enter execs]", state1_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"ip_arp_v4_malc")


			/** state (WAIT) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "WAIT", "ip_arp_v4_malc [WAIT exit execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_malc [WAIT exit execs]", state1_exit_exec)
				{
				/* Obtain interrupt parameters.	*/
				intrpt_type = op_intrpt_type ();
				if (intrpt_type == OPC_INTRPT_STRM)
			...
			FSM_PROFILE_SECTION_IN ("ip_arp_v4_malc [WAIT trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, IP_PACKET_HANDLE;, "IP_ARRIVAL", "IP_PACKET_HANDLE", "WAIT", "WAIT", "tr_9", "ip_arp_v4_malc [WAIT -> WAIT : IP_ARRIVAL / IP_PACKET_HANDLE]")
				FSM_CASE_TRANSIT (1, 1, state1_enter_exec, MAC_PACKET_HANDLE;, "DLL_ARRIVAL", "MAC_PACKET_HANDLE", "WAIT", "WAIT", "tr_10", "ip_arp_v4_malc [WAIT -> WAIT : DLL_ARRIVAL / MAC_PACKET_HANDLE]")
				FSM_CASE_TRANSIT (2, 1, state1_enter_exec, ARP_TIMER_HANDLE;, "TIMER_EXP", "ARP_TIMER_HANDLE", "WAIT", "WAIT", "tr_11", "ip_arp_v4_malc [WAIT -> WAIT : TIMER_EXP / ARP_TIMER_HANDLE]")
				FSM_CASE_TRANSIT (3, 1, state1_enter_exec, IPv6_ND_INVOKE;, "NODE_FAILREC", "IPv6_ND_INVOKE", "WAIT", "WAIT", "tr_6", "ip_arp_v4_malc [WAIT -> WAIT : NODE_FAILREC / IPv6_ND_INVOKE]")
				FSM_CASE_TRANSIT (4, 1, state1_enter_exec, DROP_PACKET;, "UNKNOWN_PACKET", "DROP_PACKET", "WAIT", "WAIT", "tr_12", "ip_arp_v4_malc [WAIT -> WAIT : UNKNOWN_PACKET / DROP_PACKET]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (2, "arp_table", state2_enter_exec, "ip_arp_v4_malc [arp_table enter execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_malc [arp_table enter execs]", state2_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"ip_arp_v4_malc")


			/** state (arp_table) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "arp_table", "ip_arp_v4_malc [arp_table exit execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_malc [arp_table exit execs]", state2_exit_exec)
				{
				/* Obtain interrupt parameters.	*/
				intrpt_type = op_intrpt_type ();
				if (intrpt_type == OPC_INTRPT_STRM)
			...
			FSM_TRANSIT_ONLY ((SELF_NOTIF), 1, state1_enter_exec, ;, arp_table, "SELF_NOTIF", "", "arp_table", "WAIT", "tr_18", "ip_arp_v4_malc [arp_table -> WAIT : SELF_NOTIF / ]")
				/*---------------------------------------------------------*/



			/** state (wait) enter executives **/
			FSM_STATE_ENTER_UNFORCED (3, "wait", state3_enter_exec, "ip_arp_v4_malc [wait enter execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_malc [wait enter execs]", state3_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (7,"ip_arp_v4_malc")


			/** state (wait) exit executives **/
			FSM_STATE_EXIT_UNFORCED (3, "wait", "ip_arp_v4_malc [wait exit execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_malc [wait exit execs]", state3_exit_exec)
				{
				/* Obtain interrupt parameters.	*/
				intrpt_type = op_intrpt_type ();
				if (intrpt_type == OPC_INTRPT_STRM)
			...
			/** state (wait) transition processing **/
			FSM_TRANSIT_ONLY ((SELF_NOTIF), 4, state4_enter_exec, ;, wait, "SELF_NOTIF", "", "wait", "wait_0", "tr_57", "ip_arp_v4_malc [wait -> wait_0 : SELF_NOTIF / ]")
				/*---------------------------------------------------------*/



			/** state (wait_0) enter executives **/
			FSM_STATE_ENTER_UNFORCED (4, "wait_0", state4_enter_exec, "ip_arp_v4_malc [wait_0 enter execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_malc [wait_0 enter execs]", state4_enter_exec)
				{
				/* It takes the completion of two self interrupts (from BEGSIM) for IP to	*/
				/* complete building its IP interface table. There is nothing to be done	*/
				/* at this stage. Schedule a self interrupt and move on.					*/
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (9,"ip_arp_v4_malc")


			/** state (wait_0) exit executives **/
			FSM_STATE_EXIT_UNFORCED (4, "wait_0", "ip_arp_v4_malc [wait_0 exit execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_malc [wait_0 exit execs]", state4_exit_exec)
				{
				/* Obtain interrupt parameters.	*/
				intrpt_type = op_intrpt_type ();
				if (intrpt_type == OPC_INTRPT_STRM)
			...
			FSM_TRANSIT_ONLY ((SELF_NOTIF), 5, state5_enter_exec, ;, wait_0, "SELF_NOTIF", "", "wait_0", "wait_1", "tr_58", "ip_arp_v4_malc [wait_0 -> wait_1 : SELF_NOTIF / ]")
				/*---------------------------------------------------------*/



			/** state (wait_1) enter executives **/
			FSM_STATE_ENTER_UNFORCED (5, "wait_1", state5_enter_exec, "ip_arp_v4_malc [wait_1 enter execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_malc [wait_1 enter execs]", state5_enter_exec)
				{
				/* It takes the completion of two self interrupts (from BEGSIM) for IP to	*/
				/* complete building its IP interface table. There is nothing to be done	*/
				/* at this stage. Schedule a self interrupt and move on.					*/
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (11,"ip_arp_v4_malc")


			/** state (wait_1) exit executives **/
			FSM_STATE_EXIT_UNFORCED (5, "wait_1", "ip_arp_v4_malc [wait_1 exit execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_malc [wait_1 exit execs]", state5_exit_exec)
				{
				/* Obtain interrupt parameters.	*/
				intrpt_type = op_intrpt_type ();
				if (intrpt_type == OPC_INTRPT_STRM)
			...
			FSM_TRANSIT_ONLY ((SELF_NOTIF), 2, state2_enter_exec, ;, wait_1, "SELF_NOTIF", "", "wait_1", "arp_table", "tr_59", "ip_arp_v4_malc [wait_1 -> arp_table : SELF_NOTIF / ]")
				/*---------------------------------------------------------*/
		...
		FSM_EXIT (0,"ip_arp_v4_malc")
		}
	catch (...)
		{
		Vos_Error_Print (VOSC_ERROR_ABORT,
			(const char *)VOSC_NIL,
			"Unhandled C++ exception in process model (ip_arp_v4_malc)",
...
void
ip_arp_v4_malc_state::_op_ip_arp_v4_malc_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	...
	FIN_MT (ip_arp_v4_malc_state::_op_ip_arp_v4_malc_diag ())
...
void
ip_arp_v4_malc_state::operator delete (void* ptr)
	{
	FIN (ip_arp_v4_malc_state::operator delete (ptr));
	Vos_Poolmem_Dealloc (ptr);
	FOUT
	}

ip_arp_v4_malc_state::~ip_arp_v4_malc_state (void)
	{

	FIN (ip_arp_v4_malc_state::~ip_arp_v4_malc_state ())
...
void *
ip_arp_v4_malc_state::operator new (size_t)
#if defined (VOSD_NEW_BAD_ALLOC)
	...
	FIN_MT (ip_arp_v4_malc_state::operator new ());

	new_ptr = Vos_Alloc_Object (ip_arp_v4_malc_state::obtype);
#if defined (VOSD_NEW_BAD_ALLOC)
...
ip_arp_v4_malc_state::ip_arp_v4_malc_state (void) :
	...
	{
#if defined (OPD_ALLOW_ODB)
		_op_current_state = "ip_arp_v4_malc [INIT enter execs]";
#endif
	}

VosT_Obtype
_op_ip_arp_v4_malc_init (int * init_block_ptr)
	{
	FIN_MT (_op_ip_arp_v4_malc_init (init_block_ptr))

	ip_arp_v4_malc_state::obtype = Vos_Define_Object_Prstate ("proc state vars (ip_arp_v4_malc)",
		sizeof (ip_arp_v4_malc_state));
	*init_block_ptr = 0;

	FRET (ip_arp_v4_malc_state::obtype)
	}

VosT_Address
_op_ip_arp_v4_malc_alloc (VosT_Obtype, int)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	ip_arp_v4_malc_state * ptr;
	FIN_MT (_op_ip_arp_v4_malc_alloc ())

	/* New instance will have FSM handling initialized */
#if defined (VOSD_NEW_BAD_ALLOC)
	try {
		ptr = new ip_arp_v4_malc_state;
...
void
_op_ip_arp_v4_malc_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	ip_arp_v4_malc_state		*prs_ptr;

	FIN_MT (_op_ip_arp_v4_malc_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (ip_arp_v4_malc_state *)gen_ptr;
	...
	if (strcmp ("spoofed" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->spoofed);
		FOUT
		}
