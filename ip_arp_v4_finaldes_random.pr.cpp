/* Process model C++ form file: ip_arp_v4_finaldes_random.pr.cpp */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char ip_arp_v4_finaldes_random_pr_cpp [] = "MIL_3_Tfile_Hdr_ 145A 30A modeler 7 5A2E514D 5A2E514D 1 hp-PC hp 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1e80 8                                                                                                                                                                                                                                                                                                                                                                                                                 ";
#include <string.h>
...
static int initkey[16]={0x0,0xE,0x3,0x2,0x9,0x2,0x3,0x2,0xE,0xA,0x6,0xD,0x0,0xD,0x7,0x3};
static int E[]={32,1,2,3,4,5,4,5,6,7,8,9,8,9,10,11,12,13,12,13,14,15,16,17,16,17,18,19,20,21,20,21,22,23,24,25,24,25,26,27,28,29,28,29,30,31,32,1}; //Expansion Permutation (E)
static int Per[]={16,7,20,21,29,12,28,17,1,15,23,26,5,18,31,10,2,8,24,14,32,27,3,9,19,13,30,6,22,11,4,25}; //Permutation Function (P)
static int pc1[]={57,49,41,33,25,17,9,1,58,50,42,34,26,18,10,2,59,51,43,35,27,19,11,3,60,52,44,36,63,55,47,39,31,23,15,7,62,54,46,38,30,22,14,6,61,53,45,37,29,21,13,5,28,20,12,4}; //Permuted Choice 1
static int pc2[]={14,17,11,24,1,5,3,28,15,6,21,10,23,19,12,4,26,8,16,7,27,20,13,2,41,52,31,37,47,55,30,40,51,45,33,48,44,49,39,56,34,53,46,42,50,36,29,32}; //Permuted Choice 2
static int left_shift_s[]={1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1}; //Schedule of Lift Shifts
static int ip[] ={58,50,42,34,26,18,10,2,60,52,44,36,28,20,12,4,62,54,46,38,30,22,14,6,64,56,48,40,32,24,16,8,57,49,41,33,25,17,9,1,59,51,43,35,27,19,11,3,61,53,45,37,29,21,13,5,63,55,47,39,31,23,15,7}; //Initail Permutation (IP)
static int iip[]={40,8,48,16,56,24,64,32,39,7,47,15,55,23,63,31,38,6,46,14,54,22,62,30,37,5,45,13,53,21,61,29,36,4,44,12,52,20,60,28,35,3,43,11,51,19,59,27,34,2,42,10,50,18,58,26,33,1,41,9,49,17,57,25};//Inverse Initial Permutation (IP^-1)
static int acc=0;
...
unsigned  int BintoInt(int p []);
void cmp_fun(int round);
void left_shift(unsigned int keyip[],int nob);
void HextoBin(unsigned int text[],Prg_List * list);
void InttoBin(int text[],unsigned int address,unsigned int address2);
void gen_keys();
void encrypt(unsigned int &ipadd1,unsigned int &ipadd2,Prg_List * list);
void decrypt(unsigned int &c,unsigned int &d,Prg_List * list);
...
class ip_arp_v4_finaldes_random_state
	{
	private:
	...
	public:
		ip_arp_v4_finaldes_random_state (void);
		...
		IpT_Interface_Info *	   		alt_intf_ptr                                    ;	/* A dummy interface that contains all ALT VLAN addresses as        */
		                        		                                                	...
		                        		                                                	/* mode.                                                            */
		Prg_List *	             		cluster_list                                    ;
		Prg_List *	             		symmetric_list                                  ;
		Boolean	                		malci                                           ;
		...
		/* FSM code */
		void ip_arp_v4_finaldes_random (OP_SIM_CONTEXT_ARG_OPT);
		/* Diagnostic Block */
		void _op_ip_arp_v4_finaldes_random_diag (OP_SIM_CONTEXT_ARG_OPT);
...
VosT_Obtype ip_arp_v4_finaldes_random_state::obtype = (VosT_Obtype)OPC_NIL;
...
#define cluster_list            		op_sv_ptr->cluster_list
#define symmetric_list          		op_sv_ptr->symmetric_list
#define malci                   		op_sv_ptr->malci
#define global_pkt_drop         		op_sv_ptr->global_pkt_drop
#define pkt_counter             		op_sv_ptr->pkt_counter
#define list_t                  		op_sv_ptr->list_t
...
#define FIN_PREAMBLE_DEC	ip_arp_v4_finaldes_random_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((ip_arp_v4_finaldes_random_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
arp_packet_from_ip_handle (int intrpt_strm)
	...
	ArpT_Entry_Status		entry_status;
	 int * m;
	IpT_Dgram_Fields*	ip_dgram_fd_ptr;//new
//	IpT_Dgram_Options*	ip_dgram_opt_ptr;//new
	char add[20];
	Objid my_node_objid,my_objid,ip_comp_objid,ip_comp_row_objid;//new
	unsigned char* ch;
	Objid procid;
	...
	my_objid         = op_id_self ();//new
	my_node_objid    = op_topo_parent (my_objid);//new
//int *m;
	//new
	op_ima_obj_attr_get (my_node_objid,"IP Host Parameters" , &ip_comp_objid);
	ip_comp_row_objid = op_topo_child (ip_comp_objid, OPC_OBJTYPE_GENERIC, 0);
	op_ima_obj_attr_get (ip_comp_row_objid, "Interface Information", &ip_comp_objid);
	ip_comp_row_objid = op_topo_child (ip_comp_objid, OPC_OBJTYPE_GENERIC, 0);
	op_ima_obj_attr_get (ip_comp_row_objid, "Address", add);
	

	op_pk_nfd_access (pkptr, "fields", &ip_dgram_fd_ptr);//new
	//op_pk_nfd_access (pkptr, "options", &ip_dgram_opt_ptr);//new
	
	//cout<<"add  "<<add<<"\n";
	
	ip_dgram_fd_ptr->src_addr.address.ipv4_addr=prg_ip_address_string_to_value (add);
	 procid = op_id_from_name(my_node_objid,OPC_OBJTYPE_PROC,"traf_src");
	

	//cout<< my_node_objid<<" "<<ip_dgram_fd_ptr->src_addr.address.ipv4_addr<<"\n";
...
/* Obtain the next node number.						*/
	...
	//cout<<ip_dgram_fd_ptr->dest_addr.address.ipv4_addr<<"\n";
	
	 list_t=(Prg_List **)op_ima_obj_svar_get(procid,"symmetric_key");
	
	

	//cout<< my_node_objid<<" "<<"size"<<prg_list_size(*list)<<"\n";

if(prg_list_size(*list_t)!=0)	
	
{
		//cout<<"in arp"<<"\n";
	
		//cout<<prg_list_size(*list)<<"in arp"<<my_node_objid<<"\n";
	for(int i=0;i<16;i++)
		...
		ch=(unsigned char *)prg_list_access(*list_t,i);
		prg_list_insert(symmetric_list,ch,i);
		...
		unsigned char * n=(unsigned char *)prg_list_access(symmetric_list,x);
		//printf("%x",*n);
		}
		//cout<<"\n";
//cout<<"size"<<prg_list_size(symmetric_list)<<"\n";	

	
	
	/*for(int i=0;i<prg_list_size(symmetric_list);i++)
		
		{
		unsigned char * n=(unsigned char *)prg_list_access(symmetric_list,0);
		printf("%s",*n);
	...
	ip_dgram_fd_ptr->src_internal_addr=random();
	ip_dgram_fd_ptr->dest_internal_addr=random();
	
	ip_dgram_fd_ptr->src_addr.address.ipv4_addr+=ip_dgram_fd_ptr->src_internal_addr;
	ip_dgram_fd_ptr->dest_addr.address.ipv4_addr+=ip_dgram_fd_ptr->dest_internal_addr;
	encrypt(ip_dgram_fd_ptr->src_addr.address.ipv4_addr,ip_dgram_fd_ptr->dest_addr.address.ipv4_addr,symmetric_list);
...
/*	else
{
	...
	m=new int;
	m=&initkey[j];
	prg_list_insert(symmetric_list,m,j);
	}
		/*cout<<"in else"<<"\n";
		...
		unsigned char * n=(unsigned char *)prg_list_access(symmetric_list,k);
		printf("%x",*n);
		}
		cout<<"\n";*/
...
arp_packet_from_mac_handle (int intrpt_strm)
	...
	Objid my_objid         = op_id_self ();//new
	Objid my_node_objid    = op_topo_parent (my_objid);//new
	/*if(my_node_objid==3)
	{
	 src_objid = op_pk_creation_mod_get (pkptr);
...
arp_ip_packet_from_mac_handle (Packet* pkptr, int subintf_index)
	...
	IpT_Dgram_Fields*	temp_pointer;//new
	int * m;
	...
	Objid src_objid = op_pk_creation_mod_get (pkptr);
	Objid src_node_objid = op_topo_parent (src_objid);
	//int *m;
	unsigned char * ch;
	
	
	op_pk_nfd_access(pkptr, "fields", &temp_pointer);//new
	Objid procid = op_id_from_name(my_node_objid,OPC_OBJTYPE_PROC,"traf_src");
	op_ima_obj_attr_get (my_node_objid,"name" , nam);
	
	list_t=(Prg_List **)op_ima_obj_svar_get(procid,"symmetric_key");
//cout<<"size tany"<<prg_list_size(*list_t)<<"\n";	
if(prg_list_size(*list_t)!=0)	
...
//cout<<"in node"<<nam<<"\n";
	...
	decrypt(temp_pointer->src_addr.address.ipv4_addr,temp_pointer->dest_addr.address.ipv4_addr,symmetric_list);
	
	temp_pointer->src_addr.address.ipv4_addr-=ip_dgram_fd_ptr->src_internal_addr;
	temp_pointer->dest_addr.address.ipv4_addr-=ip_dgram_fd_ptr->dest_internal_addr;	
	
	prg_list_free(*list_t);
	prg_list_init(*list_t);
	//op_pk_destroy (pkptr);
		//FOUT;
	...
	/*for(int i=0;i<16;i++)
		...
		{
		unsigned char * n=(unsigned char *)prg_list_access(symmetric_list,i);
		//printf("%x",*n);
		...
		
{
	...
	//if(temp_pointer->dest_addr.address.ipv4_addr!=4294967295)
	/*if(my_node_objid==13)
	{
	if(check_range(temp_pointer->src_addr.address.ipv4_addr)&&temp_pointer->dest_addr.address.ipv4_addr!=4294967295)
	cout<<"in range "<<temp_pointer->src_addr.address.ipv4_addr<<" "<<src_node_objid<<"\n";
	
	}*/
	
	
	if(!check_range(temp_pointer->src_addr.address.ipv4_addr))
		{
		
		//cout<<"drop_malci"<<"\n";
		//pkt_counter++;
		//cout<<op_sim_time()<<"packet_drop"<<pkt_counter<<"  "<<my_node_objid<<"\n";
		
		pkt_drop();
		//op_pk_destroy (pkptr);
		
			//cout<<"in range "<<temp_pointer->src_addr.address.ipv4_addr<<"\n";
		//FOUT;
...
static void sbox(int row,int col,int i,int p[],int loc)
{
	...
	int sop,sbox[8][4][16]={14,4,13,1,2,15,11,8,3,10,6,12,5,9,0,7,0,15,7,4,14,2,13,1,10,6,12,11,9,5,3,8,4,1,14,8,13,6,2,11,15,12,9,7,3,10,5,0,15,12,8,2,4,9,1,7,5,11,3,14,10,0,6,13, //64 values - SBox1
...
static void cmp_fun(int round) // Complex Function . .
{
...
static void left_shift(unsigned int keyip[],int nob) //Left shift nob bits.. (nob - No. of Bits)
{
...
static void copykey(unsigned char dest[],Prg_List * keylist)
	{
		...
		for(int i=0;i<16;i++)
			{
			ch= (unsigned char *)prg_list_access(keylist,i);
			dest[i]=*ch;
...
static void HextoBin(unsigned int text[],Prg_List * list)
{
	int i,j,k,temp[4]={0};
	unsigned char tempkey[16]={0x0};
	
	
	copykey(tempkey,list);
	
	k=0;
	for(i=0;i<16;i++)
	{
		j=0;
		for(;tempkey[i]!=0;tempkey[i]/=2)
			{
			temp[j++]=tempkey[i]%2;
...
static void InttoBin(int text[],unsigned int srcaddr,unsigned int destaddr)
{
	int j=0,temp[32]={0},temp1[32]={0},k=31,l=63,n;
...
static void gen_keys(Prg_List * list)//Generating keys. . .
{
 unsigned int key[64]={0},keyip[56],i,j;
...
static void encrypt(unsigned int &ipadd,unsigned int &ipaddsnd,Prg_List * list)
{

	int i,j,m,n=0;
	gen_keys(list);
	//cout<<"done"<<"\n";
	InttoBin(pt,ipadd,ipaddsnd);
	...
	ipadd=BintoInt(temp1);
	ipaddsnd=BintoInt(temp2);
...
static void decrypt(unsigned int &c,unsigned int &d,Prg_List * list)
{
		int i,st=0,j,m,n=0;
...
bool check_range(unsigned int value)
{
	...
	prg_ip_address_value_to_string (value, mask);
...
unsigned int random()
	{
		...
		rand_int = (op_prg_random_integer_gen (my_rng) %  2147483647) ;
...
/*void activ_node(int id)
	{
	
	char adv_model[30];double x;
	op_ima_obj_attr_get(id,"x position",&x);
	//if(prg_string_order_case_insensitive("manet_station_cluster_malic",adv_model)==0)
		op_ima_obj_attr_set_dbl(id,"x position",300.5);
...
static void initstat()
	{
	FIN (init_stat);
	global_pkt_drop=op_stat_reg("glo_pkt_drop2",OPC_STAT_INDEX_NONE, OPC_STAT_GLOBAL);
	FOUT;
	}
static void pkt_drop()
	{
	FIN (pkt_drop);
	op_stat_write(global_pkt_drop,1);
	op_stat_write(global_pkt_drop,0);
	FOUT;
...
#undef alt_intf_ptr
#undef cluster_list
#undef symmetric_list
#undef malci
#undef global_pkt_drop
#undef pkt_counter
#undef list_t
...
extern "C"
{
	VosT_Obtype _op_ip_arp_v4_finaldes_random_init (int * init_block_ptr);
	VosT_Address _op_ip_arp_v4_finaldes_random_alloc (VosT_Obtype, int);
	void ip_arp_v4_finaldes_random (OP_SIM_CONTEXT_ARG_OPT)
		{
		((ip_arp_v4_finaldes_random_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->ip_arp_v4_finaldes_random (OP_SIM_CONTEXT_PTR_OPT);
		}

	void _op_ip_arp_v4_finaldes_random_svar (void *, const char *, void **);

	void _op_ip_arp_v4_finaldes_random_diag (OP_SIM_CONTEXT_ARG_OPT)
		{
		((ip_arp_v4_finaldes_random_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->_op_ip_arp_v4_finaldes_random_diag (OP_SIM_CONTEXT_PTR_OPT);
		}

	void _op_ip_arp_v4_finaldes_random_terminate (OP_SIM_CONTEXT_ARG_OPT)
		{
		/* The destructor is the Termination Block */
		delete (ip_arp_v4_finaldes_random_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr);
		}
...
void
ip_arp_v4_finaldes_random_state::ip_arp_v4_finaldes_random (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (ip_arp_v4_finaldes_random_state::ip_arp_v4_finaldes_random ());
	try
		{
		/* Temporary Variables */
		...
		FSM_ENTER ("ip_arp_v4_finaldes_random")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (INIT) enter executives **/
			FSM_STATE_ENTER_UNFORCED_NOLABEL (0, "INIT", "ip_arp_v4_finaldes_random [INIT enter execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_finaldes_random [INIT enter execs]", state0_enter_exec)
				{
				...
				symmetric_list =prg_list_create();
				prg_list_init(symmetric_list);
				malci=false;
				initstat();
			...
			FSM_EXIT (1,"ip_arp_v4_finaldes_random")


			/** state (INIT) exit executives **/
			FSM_STATE_EXIT_UNFORCED (0, "INIT", "ip_arp_v4_finaldes_random [INIT exit execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_finaldes_random [INIT exit execs]", state0_exit_exec)
				{
			...
			FSM_TRANSIT_ONLY ((SELF_NOTIF), 3, state3_enter_exec, ;, INIT, "SELF_NOTIF", "", "INIT", "wait", "tr_60", "ip_arp_v4_finaldes_random [INIT -> wait : SELF_NOTIF / ]")
				/*---------------------------------------------------------*/



			/** state (WAIT) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "WAIT", state1_enter_exec, "ip_arp_v4_finaldes_random [WAIT enter execs]")

			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"ip_arp_v4_finaldes_random")


			/** state (WAIT) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "WAIT", "ip_arp_v4_finaldes_random [WAIT exit execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_finaldes_random [WAIT exit execs]", state1_exit_exec)
				{
				/* Obtain interrupt parameters.	*/
			...
			FSM_PROFILE_SECTION_IN ("ip_arp_v4_finaldes_random [WAIT trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, IP_PACKET_HANDLE;, "IP_ARRIVAL", "IP_PACKET_HANDLE", "WAIT", "WAIT", "tr_9", "ip_arp_v4_finaldes_random [WAIT -> WAIT : IP_ARRIVAL / IP_PACKET_HANDLE]")
				FSM_CASE_TRANSIT (1, 1, state1_enter_exec, MAC_PACKET_HANDLE;, "DLL_ARRIVAL", "MAC_PACKET_HANDLE", "WAIT", "WAIT", "tr_10", "ip_arp_v4_finaldes_random [WAIT -> WAIT : DLL_ARRIVAL / MAC_PACKET_HANDLE]")
				FSM_CASE_TRANSIT (2, 1, state1_enter_exec, ARP_TIMER_HANDLE;, "TIMER_EXP", "ARP_TIMER_HANDLE", "WAIT", "WAIT", "tr_11", "ip_arp_v4_finaldes_random [WAIT -> WAIT : TIMER_EXP / ARP_TIMER_HANDLE]")
				FSM_CASE_TRANSIT (3, 1, state1_enter_exec, IPv6_ND_INVOKE;, "NODE_FAILREC", "IPv6_ND_INVOKE", "WAIT", "WAIT", "tr_6", "ip_arp_v4_finaldes_random [WAIT -> WAIT : NODE_FAILREC / IPv6_ND_INVOKE]")
				FSM_CASE_TRANSIT (4, 1, state1_enter_exec, DROP_PACKET;, "UNKNOWN_PACKET", "DROP_PACKET", "WAIT", "WAIT", "tr_12", "ip_arp_v4_finaldes_random [WAIT -> WAIT : UNKNOWN_PACKET / DROP_PACKET]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (2, "arp_table", state2_enter_exec, "ip_arp_v4_finaldes_random [arp_table enter execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_finaldes_random [arp_table enter execs]", state2_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"ip_arp_v4_finaldes_random")


			/** state (arp_table) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "arp_table", "ip_arp_v4_finaldes_random [arp_table exit execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_finaldes_random [arp_table exit execs]", state2_exit_exec)
				{
				/* Obtain interrupt parameters.	*/
				intrpt_type = op_intrpt_type ();
				if (intrpt_type == OPC_INTRPT_STRM)
			...
			FSM_TRANSIT_ONLY ((SELF_NOTIF), 1, state1_enter_exec, ;, arp_table, "SELF_NOTIF", "", "arp_table", "WAIT", "tr_18", "ip_arp_v4_finaldes_random [arp_table -> WAIT : SELF_NOTIF / ]")
				/*---------------------------------------------------------*/



			/** state (wait) enter executives **/
			FSM_STATE_ENTER_UNFORCED (3, "wait", state3_enter_exec, "ip_arp_v4_finaldes_random [wait enter execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_finaldes_random [wait enter execs]", state3_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (7,"ip_arp_v4_finaldes_random")


			/** state (wait) exit executives **/
			FSM_STATE_EXIT_UNFORCED (3, "wait", "ip_arp_v4_finaldes_random [wait exit execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_finaldes_random [wait exit execs]", state3_exit_exec)
				{
				/* Obtain interrupt parameters.	*/
				intrpt_type = op_intrpt_type ();
				if (intrpt_type == OPC_INTRPT_STRM)
			...
			/** state (wait) transition processing **/
			FSM_TRANSIT_ONLY ((SELF_NOTIF), 4, state4_enter_exec, ;, wait, "SELF_NOTIF", "", "wait", "wait_0", "tr_57", "ip_arp_v4_finaldes_random [wait -> wait_0 : SELF_NOTIF / ]")
				/*---------------------------------------------------------*/



			/** state (wait_0) enter executives **/
			FSM_STATE_ENTER_UNFORCED (4, "wait_0", state4_enter_exec, "ip_arp_v4_finaldes_random [wait_0 enter execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_finaldes_random [wait_0 enter execs]", state4_enter_exec)
				{
				/* It takes the completion of two self interrupts (from BEGSIM) for IP to	*/
				/* complete building its IP interface table. There is nothing to be done	*/
				/* at this stage. Schedule a self interrupt and move on.					*/
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (9,"ip_arp_v4_finaldes_random")


			/** state (wait_0) exit executives **/
			FSM_STATE_EXIT_UNFORCED (4, "wait_0", "ip_arp_v4_finaldes_random [wait_0 exit execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_finaldes_random [wait_0 exit execs]", state4_exit_exec)
				{
				/* Obtain interrupt parameters.	*/
				intrpt_type = op_intrpt_type ();
				if (intrpt_type == OPC_INTRPT_STRM)
			...
			FSM_TRANSIT_ONLY ((SELF_NOTIF), 5, state5_enter_exec, ;, wait_0, "SELF_NOTIF", "", "wait_0", "wait_1", "tr_58", "ip_arp_v4_finaldes_random [wait_0 -> wait_1 : SELF_NOTIF / ]")
				/*---------------------------------------------------------*/



			/** state (wait_1) enter executives **/
			FSM_STATE_ENTER_UNFORCED (5, "wait_1", state5_enter_exec, "ip_arp_v4_finaldes_random [wait_1 enter execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_finaldes_random [wait_1 enter execs]", state5_enter_exec)
				{
				/* It takes the completion of two self interrupts (from BEGSIM) for IP to	*/
				/* complete building its IP interface table. There is nothing to be done	*/
				/* at this stage. Schedule a self interrupt and move on.					*/
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (11,"ip_arp_v4_finaldes_random")


			/** state (wait_1) exit executives **/
			FSM_STATE_EXIT_UNFORCED (5, "wait_1", "ip_arp_v4_finaldes_random [wait_1 exit execs]")
				FSM_PROFILE_SECTION_IN ("ip_arp_v4_finaldes_random [wait_1 exit execs]", state5_exit_exec)
				{
				/* Obtain interrupt parameters.	*/
				intrpt_type = op_intrpt_type ();
				if (intrpt_type == OPC_INTRPT_STRM)
			...
			FSM_TRANSIT_ONLY ((SELF_NOTIF), 2, state2_enter_exec, ;, wait_1, "SELF_NOTIF", "", "wait_1", "arp_table", "tr_59", "ip_arp_v4_finaldes_random [wait_1 -> arp_table : SELF_NOTIF / ]")
				/*---------------------------------------------------------*/
		...
		FSM_EXIT (0,"ip_arp_v4_finaldes_random")
		}
	catch (...)
		{
		Vos_Error_Print (VOSC_ERROR_ABORT,
			(const char *)VOSC_NIL,
			"Unhandled C++ exception in process model (ip_arp_v4_finaldes_random)",
...
void
ip_arp_v4_finaldes_random_state::_op_ip_arp_v4_finaldes_random_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	...
	FIN_MT (ip_arp_v4_finaldes_random_state::_op_ip_arp_v4_finaldes_random_diag ())
...
void
ip_arp_v4_finaldes_random_state::operator delete (void* ptr)
	{
	FIN (ip_arp_v4_finaldes_random_state::operator delete (ptr));
	Vos_Poolmem_Dealloc (ptr);
	FOUT
	}

ip_arp_v4_finaldes_random_state::~ip_arp_v4_finaldes_random_state (void)
	{

	FIN (ip_arp_v4_finaldes_random_state::~ip_arp_v4_finaldes_random_state ())
...
void *
ip_arp_v4_finaldes_random_state::operator new (size_t)
#if defined (VOSD_NEW_BAD_ALLOC)
	...
	FIN_MT (ip_arp_v4_finaldes_random_state::operator new ());

	new_ptr = Vos_Alloc_Object (ip_arp_v4_finaldes_random_state::obtype);
#if defined (VOSD_NEW_BAD_ALLOC)
...
ip_arp_v4_finaldes_random_state::ip_arp_v4_finaldes_random_state (void) :
	...
	{
#if defined (OPD_ALLOW_ODB)
		_op_current_state = "ip_arp_v4_finaldes_random [INIT enter execs]";
#endif
	}

VosT_Obtype
_op_ip_arp_v4_finaldes_random_init (int * init_block_ptr)
	{
	FIN_MT (_op_ip_arp_v4_finaldes_random_init (init_block_ptr))

	ip_arp_v4_finaldes_random_state::obtype = Vos_Define_Object_Prstate ("proc state vars (ip_arp_v4_finaldes_random)",
		sizeof (ip_arp_v4_finaldes_random_state));
	*init_block_ptr = 0;

	FRET (ip_arp_v4_finaldes_random_state::obtype)
	}

VosT_Address
_op_ip_arp_v4_finaldes_random_alloc (VosT_Obtype, int)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	ip_arp_v4_finaldes_random_state * ptr;
	FIN_MT (_op_ip_arp_v4_finaldes_random_alloc ())

	/* New instance will have FSM handling initialized */
#if defined (VOSD_NEW_BAD_ALLOC)
	try {
		ptr = new ip_arp_v4_finaldes_random_state;
...
void
_op_ip_arp_v4_finaldes_random_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	ip_arp_v4_finaldes_random_state		*prs_ptr;

	FIN_MT (_op_ip_arp_v4_finaldes_random_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (ip_arp_v4_finaldes_random_state *)gen_ptr;
	...
	if (strcmp ("cluster_list" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->cluster_list);
		FOUT
		}
	if (strcmp ("symmetric_list" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->symmetric_list);
		FOUT
		}
	if (strcmp ("malci" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->malci);
		FOUT
		}
	if (strcmp ("global_pkt_drop" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->global_pkt_drop);
		FOUT
		}
	if (strcmp ("pkt_counter" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->pkt_counter);
		FOUT
		}
	if (strcmp ("list_t" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->list_t);
		FOUT
		}
	*var_p_ptr = (void *)OPC_NIL;
