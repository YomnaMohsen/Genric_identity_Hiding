/* Process model C++ form file: manet_dispatcher_key_recv.pr.cpp */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char manet_dispatcher_key_recv_pr_cpp [] = "MIL_3_Tfile_Hdr_ 145A 30A modeler 7 58F392EA 58F392EA 1 hp-PC hp 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1e80 8                                                                                                                                                                                                                                                                                                                                                                                                                 ";
#include <string.h>
...
void get_keys(unsigned char arr[]);
...
class manet_dispatcher_key_recv_state
	{
	private:
	...
	public:
		manet_dispatcher_key_recv_state (void);
		...
		/* FSM code */
		void manet_dispatcher_key_recv (OP_SIM_CONTEXT_ARG_OPT);
		/* Diagnostic Block */
		void _op_manet_dispatcher_key_recv_diag (OP_SIM_CONTEXT_ARG_OPT);
...
VosT_Obtype manet_dispatcher_key_recv_state::obtype = (VosT_Obtype)OPC_NIL;
...
#define FIN_PREAMBLE_DEC	manet_dispatcher_key_recv_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((manet_dispatcher_key_recv_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
//Done by me
	...
	FIN (get_keys (unsigned char arr[]));
	int i;
	 unsigned char * ch;
	Objid my_obj_id         = op_id_self ();
	Objid	my_node_id    = op_topo_parent (my_obj_id);
	...
	//if(my_node_id==37)
	//{
	/*cout<<prg_list_size(symmetric_key)<<" "<<op_sim_time()<<"\n";
	for(i=0;i<prg_list_size(symmetric_key);i++)
	{
		
	unsigned char * ch=(unsigned char *)prg_list_access(symmetric_key,i);
	printf("%x",*ch);
	
	}
	cout<<"\n";
	//}*/
	manet_rpg_received_stats_update (32.0+(16.0*8.0));
	FOUT;
...
extern "C"
{
	VosT_Obtype _op_manet_dispatcher_key_recv_init (int * init_block_ptr);
	VosT_Address _op_manet_dispatcher_key_recv_alloc (VosT_Obtype, int);
	void manet_dispatcher_key_recv (OP_SIM_CONTEXT_ARG_OPT)
		{
		((manet_dispatcher_key_recv_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->manet_dispatcher_key_recv (OP_SIM_CONTEXT_PTR_OPT);
		}

	void _op_manet_dispatcher_key_recv_svar (void *, const char *, void **);

	void _op_manet_dispatcher_key_recv_diag (OP_SIM_CONTEXT_ARG_OPT)
		{
		((manet_dispatcher_key_recv_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->_op_manet_dispatcher_key_recv_diag (OP_SIM_CONTEXT_PTR_OPT);
		}

	void _op_manet_dispatcher_key_recv_terminate (OP_SIM_CONTEXT_ARG_OPT)
		{
		/* The destructor is the Termination Block */
		delete (manet_dispatcher_key_recv_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr);
		}
...
void
manet_dispatcher_key_recv_state::manet_dispatcher_key_recv (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (manet_dispatcher_key_recv_state::manet_dispatcher_key_recv ());
	try
		{
		/* Temporary Variables */
		...
		FSM_ENTER ("manet_dispatcher_key_recv")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_UNFORCED_NOLABEL (0, "init", "manet_dispatcher_key_recv [init enter execs]")
				FSM_PROFILE_SECTION_IN ("manet_dispatcher_key_recv [init enter execs]", state0_enter_exec)
				{
			...
			FSM_EXIT (1,"manet_dispatcher_key_recv")


			/** state (init) exit executives **/
			FSM_STATE_EXIT_UNFORCED (0, "init", "manet_dispatcher_key_recv [init exit execs]")
				FSM_PROFILE_SECTION_IN ("manet_dispatcher_key_recv [init exit execs]", state0_exit_exec)
				{
			...
			FSM_TRANSIT_FORCE (2, state2_enter_exec, ;, "default", "", "init", "wait", "tr_31", "manet_dispatcher_key_recv [init -> wait : default / ]")
				/*---------------------------------------------------------*/



			/** state (discover) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "discover", state1_enter_exec, "manet_dispatcher_key_recv [discover enter execs]")
				FSM_PROFILE_SECTION_IN ("manet_dispatcher_key_recv [discover enter execs]", state1_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"manet_dispatcher_key_recv")


			/** state (discover) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "discover", "manet_dispatcher_key_recv [discover exit execs]")
				FSM_PROFILE_SECTION_IN ("manet_dispatcher_key_recv [discover exit execs]", state1_exit_exec)
				{
			...
			FSM_TRANSIT_FORCE (4, state4_enter_exec, ;, "default", "", "discover", "wait_2", "tr_40", "manet_dispatcher_key_recv [discover -> wait_2 : default / ]")
				/*---------------------------------------------------------*/



			/** state (wait) enter executives **/
			FSM_STATE_ENTER_UNFORCED (2, "wait", state2_enter_exec, "manet_dispatcher_key_recv [wait enter execs]")
				FSM_PROFILE_SECTION_IN ("manet_dispatcher_key_recv [wait enter execs]", state2_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"manet_dispatcher_key_recv")


			/** state (wait) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "wait", "manet_dispatcher_key_recv [wait exit execs]")


			/** state (wait) transition processing **/
			FSM_TRANSIT_FORCE (5, state5_enter_exec, ;, "default", "", "wait", "wait_0", "tr_37", "manet_dispatcher_key_recv [wait -> wait_0 : default / ]")
				/*---------------------------------------------------------*/



			/** state (dispatch) enter executives **/
			FSM_STATE_ENTER_UNFORCED (3, "dispatch", state3_enter_exec, "manet_dispatcher_key_recv [dispatch enter execs]")

			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (7,"manet_dispatcher_key_recv")


			/** state (dispatch) exit executives **/
			FSM_STATE_EXIT_UNFORCED (3, "dispatch", "manet_dispatcher_key_recv [dispatch exit execs]")
				FSM_PROFILE_SECTION_IN ("manet_dispatcher_key_recv [dispatch exit execs]", state3_exit_exec)
				{
				...
				if (intrpt_type==OPC_INTRPT_STRM)
					...
					rcvd_pkt=op_pk_get (op_intrpt_strm ());
						...
						manet_rpg_packet_destroy(rcvd_pkt);
							...
							//cout<<op_sim_time()<<" "<<"my id  "<<src_node_objid<<"\n";
			...
			FSM_PROFILE_SECTION_IN ("manet_dispatcher_key_recv [dispatch trans conditions]", state3_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 3, state3_enter_exec, manet_rpg_generate_packet ();, "SELF_INTERRUPT", "manet_rpg_generate_packet ()", "dispatch", "dispatch", "tr_36", "manet_dispatcher_key_recv [dispatch -> dispatch : SELF_INTERRUPT / manet_rpg_generate_packet ()]")
				FSM_CASE_TRANSIT (1, 3, state3_enter_exec, ;, "default", "", "dispatch", "dispatch", "tr_33", "manet_dispatcher_key_recv [dispatch -> dispatch : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (4, "wait_2", state4_enter_exec, "manet_dispatcher_key_recv [wait_2 enter execs]")
				FSM_PROFILE_SECTION_IN ("manet_dispatcher_key_recv [wait_2 enter execs]", state4_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (9,"manet_dispatcher_key_recv")


			/** state (wait_2) exit executives **/
			FSM_STATE_EXIT_UNFORCED (4, "wait_2", "manet_dispatcher_key_recv [wait_2 exit execs]")
				FSM_PROFILE_SECTION_IN ("manet_dispatcher_key_recv [wait_2 exit execs]", state4_exit_exec)
				{
			...
			FSM_TRANSIT_FORCE (3, state3_enter_exec, ;, "default", "", "wait_2", "dispatch", "tr_41", "manet_dispatcher_key_recv [wait_2 -> dispatch : default / ]")
				/*---------------------------------------------------------*/



			/** state (wait_0) enter executives **/
			FSM_STATE_ENTER_UNFORCED (5, "wait_0", state5_enter_exec, "manet_dispatcher_key_recv [wait_0 enter execs]")
				FSM_PROFILE_SECTION_IN ("manet_dispatcher_key_recv [wait_0 enter execs]", state5_enter_exec)
				{
				/** Wait so that all nodes can register their		**/
				/** own addresses in the global list of possible	**/
				/** IP destinations.								**/
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (11,"manet_dispatcher_key_recv")


			/** state (wait_0) exit executives **/
			FSM_STATE_EXIT_UNFORCED (5, "wait_0", "manet_dispatcher_key_recv [wait_0 exit execs]")
				FSM_PROFILE_SECTION_IN ("manet_dispatcher_key_recv [wait_0 exit execs]", state5_exit_exec)
				{
			...
			FSM_TRANSIT_FORCE (6, state6_enter_exec, ;, "default", "", "wait_0", "wait_1", "tr_39", "manet_dispatcher_key_recv [wait_0 -> wait_1 : default / ]")
				/*---------------------------------------------------------*/



			/** state (wait_1) enter executives **/
			FSM_STATE_ENTER_UNFORCED (6, "wait_1", state6_enter_exec, "manet_dispatcher_key_recv [wait_1 enter execs]")
				FSM_PROFILE_SECTION_IN ("manet_dispatcher_key_recv [wait_1 enter execs]", state6_enter_exec)
				{
				/** Wait so that all nodes can register their		**/
				/** own addresses in the global list of possible	**/
				/** IP destinations.								**/
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (13,"manet_dispatcher_key_recv")


			/** state (wait_1) exit executives **/
			FSM_STATE_EXIT_UNFORCED (6, "wait_1", "manet_dispatcher_key_recv [wait_1 exit execs]")
				FSM_PROFILE_SECTION_IN ("manet_dispatcher_key_recv [wait_1 exit execs]", state6_exit_exec)
				{
			...
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "wait_1", "discover", "tr_32", "manet_dispatcher_key_recv [wait_1 -> discover : default / ]")
				/*---------------------------------------------------------*/
		...
		FSM_EXIT (0,"manet_dispatcher_key_recv")
		}
	catch (...)
		{
		Vos_Error_Print (VOSC_ERROR_ABORT,
			(const char *)VOSC_NIL,
			"Unhandled C++ exception in process model (manet_dispatcher_key_recv)",
...
void
manet_dispatcher_key_recv_state::_op_manet_dispatcher_key_recv_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}

void
manet_dispatcher_key_recv_state::operator delete (void* ptr)
	{
	FIN (manet_dispatcher_key_recv_state::operator delete (ptr));
	Vos_Poolmem_Dealloc (ptr);
	FOUT
	}

manet_dispatcher_key_recv_state::~manet_dispatcher_key_recv_state (void)
	{

	FIN (manet_dispatcher_key_recv_state::~manet_dispatcher_key_recv_state ())
...
void *
manet_dispatcher_key_recv_state::operator new (size_t)
#if defined (VOSD_NEW_BAD_ALLOC)
	...
	FIN_MT (manet_dispatcher_key_recv_state::operator new ());

	new_ptr = Vos_Alloc_Object (manet_dispatcher_key_recv_state::obtype);
#if defined (VOSD_NEW_BAD_ALLOC)
...
manet_dispatcher_key_recv_state::manet_dispatcher_key_recv_state (void) :
		_op_current_block (0)
	{
		...
		_op_current_state = "manet_dispatcher_key_recv [init enter execs]";
#endif
	}

VosT_Obtype
_op_manet_dispatcher_key_recv_init (int * init_block_ptr)
	{
	FIN_MT (_op_manet_dispatcher_key_recv_init (init_block_ptr))

	manet_dispatcher_key_recv_state::obtype = Vos_Define_Object_Prstate ("proc state vars (manet_dispatcher_key_recv)",
		sizeof (manet_dispatcher_key_recv_state));
	*init_block_ptr = 0;

	FRET (manet_dispatcher_key_recv_state::obtype)
	}

VosT_Address
_op_manet_dispatcher_key_recv_alloc (VosT_Obtype, int)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	manet_dispatcher_key_recv_state * ptr;
	FIN_MT (_op_manet_dispatcher_key_recv_alloc ())

	/* New instance will have FSM handling initialized */
#if defined (VOSD_NEW_BAD_ALLOC)
	try {
		ptr = new manet_dispatcher_key_recv_state;
...
void
_op_manet_dispatcher_key_recv_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	manet_dispatcher_key_recv_state		*prs_ptr;

	FIN_MT (_op_manet_dispatcher_key_recv_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (manet_dispatcher_key_recv_state *)gen_ptr;
